...
These are accepted-as-shipped trade-offs surfaced during the CODAP-1231 PR review. Not design decisions — they're noted here so future readers see the limitation without re-deriving it. None has a regression test because none describes a current defect.

**Drag escapes `flip`/`shift` does not re-clamp on viewport resize.** While dragging, the active middleware list is `[dragOverride, arrow]` — `flip()`/`shift()` are intentionally dropped (see Decision § *Drag middleware deepEqual gotcha*). The 8px viewport-padding clamp in `usePopoverDrag.onMove` is the safety net during the gesture itself. The post-drag position is not re-clamped on `window.resize`: if the viewport shrinks after the drag completes (browser resize, devtools toggled open) the popover can land outside the new viewport. Edge case requiring deliberate effort to trigger.

**Two body-rooted `MutationObserver`s when CODAP-1232 is also active.** A live tour and the CODAP-1232 UI-change-notification flow each subscribe to the entire `document.body` subtree. They don't share a subscription, so the mutation list is traversed twice. The `useTargetWatcher` callback is a single `target.isConnected` ref read (~free), and CODAP-1232's observer has a fast-path reject for ~95% of mutations, so the actual cost is negligible. Worth flagging for anyone introducing a third body-scoped observer — at that point a shared subscription is probably justified.

**`OutlineRing` rect-equality uses raw float comparison.** The shallow-equality guard in [`outline-ring.tsx`](../v3/src/lib/tour/tour-engine/outline-ring.tsx) compares `getBoundingClientRect()` outputs as raw floats. Sub-pixel jitter (e.g. during a smooth scroll) treats two visually-identical rects as different and re-renders the ring. Below perceptual threshold and well within React's render budget; not rounded.

**Popover focus flow is split across three locations and must be edited as a unit.** Three pieces work together: (1) the iframe focus-pull `useEffect` in [`popover.tsx`](../v3/src/lib/tour/tour-engine/popover.tsx) (`document.activeElement?.tagName === "IFRAME"` → `popoverEl.focus()`); (2) `tabIndex={-1}` on the popover root; (3) the `.codap-tour-popover:focus { outline: none }` rule in [`tour-styles.scss`](../v3/src/lib/tour/tour-styles.scss). The pull exists because keydown events don't bubble out of iframes — without it the keyboard handler is dead when a plugin starts the tour. `tabindex=-1` keeps the popover off the natural tab order while still allowing programmatic focus. The outline suppression hides the visible side effect of that programmatic focus on the root (inner buttons keep their focus rings). Touch any one and the other two need to be revisited.
