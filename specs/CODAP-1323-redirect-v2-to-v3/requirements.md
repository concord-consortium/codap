...
- **R21** — The canonical post-flip V3 URL is `https://codap.concord.org/app/` (Q1 resolved 2026-05-19). The redirect function emits a **host-preserving relative** destination — exposed as a top-of-file constant `V3_BASE_URL = "/app"` (no trailing slash) — and the browser resolves it against the request origin. On prod that resolves to `https://codap.concord.org/app/`; on the pre-flip clone at `codap2to3.concord.org` it resolves to `https://codap2to3.concord.org/app/` (which serves V3 from the same `S3-Website-models-resources-codap3` origin), so pre-flip testing actually reaches V3 instead of being bounced to prod which is still V2. The function constructs the destination as `${V3_BASE_URL}/${queryString}${original_hash}` in the client-side JS, where `queryString` follows the rules in R17a (path-detected `lang` wins; remaining non-`lang` query params preserved) and the path component always ends with `/` per Q8. In this template, `queryString` is either the empty string `""` or a string beginning with a literal `?` (e.g. `?lang=fr&foo=bar`) — the leading `?` is part of `queryString`, not the template; the R17a rules describe only the parameter *content*. `original_hash` is likewise `""` or a `#`-prefixed string, as `window.location.hash` returns it. A redirect with neither a query nor a hash therefore yields exactly `/app/` — resolved by the browser to `https://codap.concord.org/app/` on prod (Q1's canonical URL) and `https://codap2to3.concord.org/app/` on the pre-flip clone. The `/app/*` cache behavior on the new distribution must target the V3 S3 origin (currently `S3-Website-models-resources-codap3` on E3H9X49AG3GYSO; same origin DomainName `models-resources.s3-website-us-east-1.amazonaws.com` with `OriginPath: /codap3`) so that post-redirect requests serve V3 content. Pointing the behavior at that origin is necessary but not sufficient: the origin serves V3 rooted at `codap3/`, so a `/app/...` request would resolve to the non-existent key `codap3/app/...`. The function strips the `/app` prefix on every non-redirected `/app/*` request (R1a) so the request resolves correctly.
