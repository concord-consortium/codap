...
context("case card", () => {
  ...
  describe("case card", () => {
    ...
    it("initially displays a summary view of all cases and whenever 'Summarize Dataset' button is clicked", () => {
      ...
      card.getAttributeValues().eq(4).should("have.text", "0.02-6,400")
      ...
      card.getAttributeValues().eq(4).should("have.text", "6,400")
    ...
    it("allows the user to add, edit, and hide attributes with undo/redo", () => {
      ...
      cy.get('[data-testid^="attribute-header-menu-"]').find("button").eq(9).trigger("click")
      card.getAttributes().should("have.length", 9)
      card.getAttributeNames().should("have.length", 9)
      card.getAttributeValues().should("have.length", 9)
      ...
      cy.get('[data-testid^="attribute-header-menu-"]').find("button").first().trigger("click")
      cy.wait(500)
...
context("case card inspector panel", () => {
  ...
  describe("case card inspector panel", () => {
    ...
    it("allows user to delete selected and unselected cases from inspector panel", () => {
      ...
      card.getDeleteCasesButton().click()
      cy.wait(500)
      cy.get('[data-testid="inspector-menu-trash"]').should("be.visible")
    ...
    it("allows user to set aside and restore cases from inspector panel", () => {
      ...
      card.getSetAsideSelectedCasesButton().should("have.attr", "aria-disabled", "true")
      // Close the hide-show menu before interacting with other elements
      ...
      card.getHideShowButton().click()
      cy.wait(500)
      card.getRestoreSetAsideCasesButton()
        .should("have.text", "Restore 0 Set Aside Cases").and("have.attr", "aria-disabled", "true")
      // resorting to {force: true} because this is failing in CI, though it passes locally
      ...
      cy.wait(500)
      card.getRestoreSetAsideCasesButton()
        .should("have.text", "Restore 1 Set Aside Cases").and("not.have.attr", "aria-disabled")
      // resorting to {force: true} because this is failing in CI, though it passes locally
    ...
    it("allows user to show hidden attributes from inspector panel", () => {
      ...
      card.getHideShowButton().click()
      cy.wait(500)
      card.getShowAllHiddenAttributesButton().should("have.attr", "aria-disabled", "true")
      // Close the hide-show menu before interacting with attributes
      cy.get("body").type("{esc}")
      cy.get("[data-testid=inspector-menu-hide-show]").should("not.exist")
      card.openAttributeMenu(8)
      cy.get('[data-testid^="attribute-header-menu-"]').should("be.visible")
      cy.get('[data-testid^="attribute-header-menu-"]').find("button").contains("Hide Attribute").click()
      card.getAttributes().should("have.length", 8)
      card.getHideShowButton().click()
      cy.get('[data-testid="inspector-menu-hide-show"]').should("be.visible")
      cy.get('[data-testid="hide-show-menu-show-all-hidden-attributes"]').should("not.have.attr", "aria-disabled")
      cy.get('[data-testid="hide-show-menu-show-all-hidden-attributes"]').click()
      card.getAttributes().should("have.length", 9)
    })
    it("allows user to add an attribute from inspector panel", () => {
      ...
      cy.wait(500)
      cy.get('[data-testid^="attribute-header-menu-"]').should("be.visible")
      cy.get('[data-testid^="attribute-header-menu-"] button').contains("Edit Formula...").click()
