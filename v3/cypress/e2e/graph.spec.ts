...
context("Graph UI", () => {
  ...
  describe("graph view", () => {
    ...
    it("should check all graph tooltips", () => {
      ...
      graph.getResizeIcon().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphResizeButton)
      })
      graph.getHideShowButton().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphHideShowButton)
      })
      graph.getDisplayValuesButton().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphDisplayValuesButton)
      })
      graph.getDisplayStylesButton().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphDisplayStylesButton)
      })
      graph.getCameraButton().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphCameraButton)
      })
      ...
      graph.getDisplayConfigButton().then($element => {
        c.checkReactAriaToolTip($element, c.tooltips.graphDisplayConfigButton)
      })
  ...
  describe("graph inspector panel", () => {
    ...
    it("should hide and show selected/unselected cases", () => {
      ...
      cy.get("[data-testid=hide-unselected-cases]").should("not.have.attr", "aria-disabled")
      cy.get("[data-testid=show-all-cases]").should("have.attr", "aria-disabled", "true")
      cy.get("[data-testid=hide-unselected-cases]").click()
      cy.wait(500)
      graph.getHideShowButton().click()
      cy.get("[data-testid=hide-selected-cases]").should("have.attr", "aria-disabled", "true")
      cy.get("[data-testid=hide-unselected-cases]").should("have.attr", "aria-disabled", "true")
      cy.get("[data-testid=show-all-cases]").should("not.have.attr", "aria-disabled")
    ...
    it("should display only selected cases and adjusts axes when 'Display Only Selected Cases' is selected", () => {
      ...
      cy.wait(500)
      graph.getHideShowButton().click()
      cy.get("[data-testid=display-selected-cases]").should("not.have.attr", "aria-disabled")
      cy.get("[data-testid=show-all-cases]").should("have.attr", "aria-disabled", "true")
      cy.get("[data-testid=display-selected-cases]").click()
      cy.wait(500)
      graph.getHideShowButton().click()
      cy.get("[data-testid=display-selected-cases]").should("have.attr", "aria-disabled", "true")
      cy.get("[data-testid=show-all-cases]").should("not.have.attr", "aria-disabled")
      cy.get("[data-testid=show-all-cases]").click()
      cy.wait(500)
      graph.getHideShowButton().click()
    ...
    it("should add a banner to the graph when Show Measures for Selection is activated", () => {
      ...
      graph.getHideShowButton().click()
      cy.get("[data-testid=show-selection-measures]").should("be.visible").click()
    ...
    it("should apply `with-parent-toggles` class to adornment banners when parent toggles are shown", () => {
      ...
      graph.getHideShowButton().click()
      cy.get("[data-testid=show-parent-toggles]").should("be.visible").click()
      graph.getHideShowButton().click()
      cy.get("[data-testid=show-selection-measures]").should("be.visible").click()
