...
context("Map UI", () => {
  ...
  it("checks all map tooltips", () => {
    ...
    map.getResizeIcon().should("exist").then($element => {
      c.checkReactAriaToolTip($element, c.tooltips.mapResizeButton)
    })
    map.getHideShowButton().then($element => {
      c.checkReactAriaToolTip($element, c.tooltips.mapHideShowButton)
    })
    map.getDisplayValuesButton().then($element => {
      c.checkReactAriaToolTip($element, c.tooltips.mapDisplayValuesButton)
    })
    map.getDisplayConfigButton().then($element => {
      c.checkReactAriaToolTip($element, c.tooltips.mapDisplayConfigButton)
    })
    map.getCameraButton().then($element => {
      c.checkReactAriaToolTip($element, c.tooltips.mapCameraButton)
    })
  })
  ...
  it("checks show/hide selected/unselected/all map boundaries", () => {
    ...
    map.getHideSelectedCases().should("have.attr", "aria-disabled", "true")
    map.getHideUnselectedCases().should("not.have.attr", "aria-disabled")
    map.getShowAllCases().should("have.attr", "aria-disabled", "true")

    map.selectHideUnselectedCases()
    map.selectHideShowButton()
    map.getHideSelectedCases().should("have.attr", "aria-disabled", "true")
    map.getHideUnselectedCases().should("have.attr", "aria-disabled", "true")
    map.getShowAllCases().should("not.have.attr", "aria-disabled")
  ...
  it.skip("checks show/hide map boundaries with legend selections", () => {
    ...
    mlh.selectCategoryNameForCategoricalLegend(arrayOfValues[0].values[0])
    map.selectHideShowButton()
    map.getHideSelectedCases().should("not.have.attr", "aria-disabled")
    map.getHideUnselectedCases().should("not.have.attr", "aria-disabled")
    map.getShowAllCases().should("have.attr", "aria-disabled", "true")
