...
context("codap toolbar", () => {
  ...
  it("will open a graph", () => {
    ...
    // graphs with no associated data set should have an empty title but show a blank space when hovered
  ...
  it('will show a list of open tiles when there is no data context', ()=>{
    ...
    toolbar.getTilesListMenuItemLabel().eq(2).should("have.text", "v1, slider")
    toolbar.getTilesListMenuIcon().eq(2).should("have.class", "CodapSlider")
    toolbar.getTilesListMenuItemLabel().eq(3).should("have.text", "Calculator, calculator")
    toolbar.getTilesListMenuIcon().eq(3).should("have.class", "Calculator")
    toolbar.getTilesListMenuItemLabel().eq(4).should("have.text", "Sampler, plugin")
  ...
  it('will show correct title for a new table', ()=>{
    ...
    toolbar.getTilesListMenuItemLabel().eq(0).should("have.text", "New Dataset, case table")
  ...
  it('will show a list of open tiles when there is a data context', ()=>{
    ...
    toolbar.getTilesListMenu().should("be.visible")
    toolbar.getTilesListMenuItem().should("have.length", 5)
    toolbar.getTilesListMenuItemLabel().eq(0).should("have.text", "Tracks/Measurements, case table")
    toolbar.getTilesListMenuItemLabel().eq(1).should("have.text", "Measurements, map")
    toolbar.getTilesListMenuItemLabel().eq(2).should("have.text", "Measurements, graph")
    toolbar.getTilesListMenuItemLabel().eq(3).should("have.text", "Getting Started, text")
    toolbar.getTilesListMenuItemLabel().eq(4).should("have.text", "Getting Started, web page")
  })
  it('will show shortcut keys in the tiles menu', ()=>{
    c.clickIconFromToolShelf("graph")
    c.getIconFromToolShelf("map").click()
    c.getIconFromToolShelf("slider").click()
    toolbar.getTilesButton().click()
    toolbar.getTilesListMenu().should("be.visible")
    toolbar.getTilesListMenuShortcutKey().should("have.length", 3)
    toolbar.getTilesListMenuShortcutKey().eq(0).should("have.text", "1")
    toolbar.getTilesListMenuShortcutKey().eq(1).should("have.text", "2")
    toolbar.getTilesListMenuShortcutKey().eq(2).should("have.text", "3")
  })
  it('will disambiguate duplicate tile names with numbered suffixes', ()=>{
    // Create two graphs — both have empty titles, same type
    c.clickIconFromToolShelf("graph")
    c.clickIconFromToolShelf("graph")
    toolbar.getTilesButton().click()
    toolbar.getTilesListMenu().should("be.visible")
  ...
  it('will select a tile via shortcut key while the menu is open', ()=>{
    ...
    c.clickIconFromToolShelf("graph")
    graph.getGraphTile(1).should("have.class", "focused")
    // Open the tiles menu and press "1" to select the first graph
    toolbar.getTilesButton().click()
    toolbar.getTilesListMenu().should("be.visible")
    cy.realPress("1")
    // Menu should close and first graph should be focused
    toolbar.getTilesButton().should("have.attr", "aria-expanded", "false")
    graph.getGraphTile(0).should("have.class", "focused")
    graph.getGraphTile(1).should("not.have.class", "focused")
  })
