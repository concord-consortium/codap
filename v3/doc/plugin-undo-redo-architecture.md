...
4. **Identity stability across tile recreation.** When a plugin creates a CODAP tile (graph, table, etc.) and the user subsequently modifies that tile, the user's modification is recorded as an undo entry containing the tile's ID (either in MST patch paths or in command closures). If the plugin's tile creation is undone and then redone, the plugin recreates the tile via the DI API, which assigns a **new** ID. Any subsequent redo of the user's modification will reference the original (now stale) ID, causing the redo to fail.

   This limitation also exists in V2 — graph modification commands store the component ID in closures, and redo fails after a graph is destroyed and recreated with a new ID. It has not manifested in practice because SageModeler (the primary standalone-mode plugin) does not make tile creation undoable — tile creation is fire-and-forget, and SageModeler's undo/redo operates exclusively on its own internal model (nodes and links), not on CODAP tiles.

   **Future resolution.** Two complementary improvements could address this:

   - **ID remapping in the undo/redo system.** Add an ID remapping layer in `Tree.applyPatchesFromManager()` (the single point where patches are applied via `applyPatch(self, patchesToApply)`) that rewrites stale tile IDs in patch paths before application. When a tile is recreated, the mapping from old ID to new ID would be registered, and subsequent patch applications would transparently resolve the correct tile. The DI API already supports component lookup by name and title in addition to ID; similar fallback resolution could inform the remapping. This is worth pursuing as a general robustness strategy — CODAP should not be brittle in the face of ID mismatches regardless of their cause.

   - **Optional `id` parameter in DI API component creation.** Allow plugins to specify a tile ID when creating a component, so that a plugin redoing a creation can reuse the original ID. The handler would need to validate that the requested ID does not already exist (returning failure for duplicates). This gives well-behaved plugins a clean path to ID-stable redo, while the remapping layer provides defense in depth for plugins that don't track IDs.
