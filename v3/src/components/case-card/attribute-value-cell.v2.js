...
DG.React.ready(function () {
  ...
  function AttributeValueCell(props) {
    ...
    var tAttr = props.attribute,
        ...
        tValue = props.displayCase?.getValue(tAttrID),
        tType = tValue?.jsonBoundaryObject
            ? 'boundary'
            : tAttr.get('type'),
        tColorValueField,
    ...
    if (tType === 'numeric' && !DG.isNumeric(tValue)) {
      tValue = props.displayCase?.getForcedNumericValue(tAttrID)
    }
    if (tValue instanceof Error) {
      ...
      var tResult = 'a boundary',
          ...
          tThumb = tBoundaryObject?.jsonBoundaryObject?.properties?.THUMB
