...
import { EditAttributePropertiesModal } from "../case-table-card-common/attribute-menu/edit-attribute-properties-modal"
import { EditFormulaModal } from "../case-table-card-common/attribute-menu/edit-formula-modal"
  ...
  var CaseCard = createReactClass(
      (function () {
        ...
        var kThresholdDistance2 = 0, // pixels^2
            tTouchID,
            tTouchTimer,
            tPointerIsDown = false,   // eslint-disable-line @typescript-eslint/no-unused-vars
            tStartCoordinates,        // eslint-disable-line @typescript-eslint/no-unused-vars
            tDragInProgress = false,
            tDragHandler              // eslint-disable-line @typescript-eslint/no-unused-vars
...
/*
        var ChangeListener = SC.Object.extend({
          ...
          guaranteeDataContextObserver (iDataContext) {
          ...
          removeDataContextObserver (iDataContext) {
            if (iDataContext)
              { iDataContext.removeObserver('changeCount', this, this.contextDataDidChange) }
          },

          contextDataDidChange (iDataContext) {
            var this_ = this
...
*/
        return {
          ...
          handleColumnWidthChange (collection, width) {
          ...
          handleUserResizeColumn (collectionName, iWidth, isComplete) {
          ...
          stashEditValueInCaseAttributeValue() {
          ...
          renderCollectionHeader (iIndex, iCollClient, iCaseID) {
          ...
          renderAttribute (iContext, iCollection, iCases,
            ...
            var toggleEditing = function (iValueField, iMoveDirection) {
              ...
              function isEditableIndex(index) {
                ...
                return (index >= 0) && (index < tAttrs.length) &&
                        (attr = tAttrs[index]) && attr.get('editable') && !attr.get('hasFormula')
              }
            ...
            var getColumnWidthPct = function(collectionName, attrIndex) {
              ...
              return this.props.columnWidthMap?.[collectionName]
            }.bind(this)
           ...
           */
          moveToCase (iCollectionClient, iCaseIndex) {
           ...
           */
          getSelectedCaseFirstChildCaseID (iCollection) {
          ...
          getSelectedCaseIndices (iCollectionClient) {
...
return tSortedSelection.map(function(iEntry) {
           ...
           * @param iCaseIndex  {
           */
          moveToPreviousCase (iCollectionClient, iCaseIndex) {
            this.stashEditValueInCaseAttributeValue()
          ...
          moveToNextCase (iCollectionClient, iCaseIndex) {
            this.stashEditValueInCaseAttributeValue()
          ...
          newCase(iCollectionClient) {
            var tCollection = iCollectionClient.get('collection'),
                tContext = this.props.context,
                tAttrIDs = tCollection?.getAttributeIDs(),
...
return tResult
                  ...
                  function getParentsOfChildmostSelection() {
                    var tParents = [],
                        tCollOfChildMostSelection = tChildmostSelection?.[0].get('collection')
                    if (tChildmostSelection?.[0].get('collection').isDescendantOf(iCollection)) {
                  ...
                  var tCaseMetadata = getSharedCaseMetadataFromDataset(tContext.data),
                      ...
                      tCollectionHeader = this.renderCollectionHeader(iCollIndex, tCollClient, tCase?.get('id')),
                      tColumnWidth = this.state.resizeWidths[tCollectionName] || this.state.columnWidths[tCollectionName],
                      tContainerWidth = this.props.size?.width,
                      tAttrEntries = [],
                      tResizeHandle
  ...
  DG.React.CaseCard = createReactFactory(CaseCard)
  // TODO: figure out how to get withSize() to work
  // DG.React.CaseCard = createReactFactory(withSize()(CaseCard))
})
