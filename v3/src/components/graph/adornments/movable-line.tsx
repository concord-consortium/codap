...
export const MovableLine = (props: {
  ...
  const {model, xScale: x, yScale: y} = props,
    ...
    pointsOnAxes = useRef(lineToAxisIntercepts(model.slope, model.intercept, xDomain, yDomain))
  ...
  useEffect(function refresh() {
      ...
      return () => disposer()
    }, [pointsOnAxes, lineObject, props.transform, x, xDomain, y, yDomain, model]
  )
    ...
    continueRotation2 = useCallback((event: { x: number, y: number, dx: number, dy: number }) => {
      if (event.dx !== 0 || event.dy !== 0) {
        ...
        const newPivot2 = {x: x.invert(event.x - 60), y: y.invert(event.y)},
          pivot1 = isFinite(model.pivot1.x) ? model.pivot1 : pointsOnAxes.current.pt1
