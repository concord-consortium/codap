...
export const DotPlotDots = memo(observer(function DotPlotDots(props: IProps) {
  ...
  const onDragStart = useCallback((event: MouseEvent) => {
    ...
    }, [dataset, dragPointRadius, primaryAttrID, primaryIsBottom, enableAnimation]),
      ...
      if (dragID) {
        ...
        if (deltaPixels !== 0) {
          ...
          const delta = Number(primaryScale?.invert(deltaPixels)) - Number(primaryScale?.invert(0)),
            caseValues: ICase[] = []
          primaryAttrID && dataset?.selection.forEach(anID => {
    ...
    }, [dataset, dragID, primaryAttrID, primaryIsBottom, primaryScale]),
    ...
    }, [dataset, selectedPointRadius, dragID, enableAnimation, primaryAttrID])
