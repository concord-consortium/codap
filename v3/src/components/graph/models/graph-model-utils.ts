  ...
  const setupAxis = (place: AxisPlace) => {
    ...
    if (newAxisModel) {
      ...
      if (isCategoricalOrColorAxisModel(newAxisModel)) {
        console.log("in graph-model-utils setupAxis isColorOrCategorical", isCategoricalOrColorAxisModel(newAxisModel))
        const categorySet = dataConfig.categorySetForAttrRole(attributeRole)
        console.log("in graph-model-utils setupAxis categorySet", categorySet)
