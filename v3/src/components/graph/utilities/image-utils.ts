...
export const graphSnapshot = (options: IGraphSnapshotOptions): Promise<string | Blob> => {
  ...
  //     }
  //   })
  //   // css.textContent.split("\n").forEach((line, index) => {
  //   //   if (line.includes("movable-value-label")) console.log(` -- css`, line)
  //   // })
  //   // console.log(`--- css text`, css.textContent)
  ...
  const makeCanvas = (bgColor: string, x: number, y: number, _width: number, _height: number): HTMLCanvasElement => {
  ...
  //     switch (elType) {
  //       case "div": {
  //         mainCtx.fillStyle = getComputedStyle(job.element).backgroundColor || "#f8f8f8"
  ...
  console.log(`--- rootel`, rootEl.outerHTML)
...
`</svg>`
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (mainCtx) {
        img.insertBefore(css, img.firstChild)
        // const svgEl = img as SVGSVGElement
        // const dataURL = makeDataURLFromSVGElement(svgEl, { height, width })
        // const svgImg = await makeSVGImage(dataURL)
      ...
      resolve(asDataURL ? mainCanvas.toDataURL("image/png") : makeCanvasBlob(mainCanvas))
    }
    img.src = data
