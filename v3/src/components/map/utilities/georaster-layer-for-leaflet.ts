    ...
    if (!this._bounds) {
      ...
      if (isSimpleCRS(this.getMapCRS())) {
        ...
        if (typeof proj4FullyLoaded !== "function") {
          throw `Without proj4, we do not support rasters with the projection ${projection}.`
        } else {
          throw `No support for rasters with the projection ${projection}.` +
            `Add the projection definition to your global proj4.`
        }
      }
