...
function PluginItem({ onClose, pluginData }: IPluginItemProps) {
  ...
  const IconComponent = iconComponents[pluginData?.icon ?? ""]
  // const iconUrl = pluginData && URL.canParse(pluginData.icon)
  //   ? pluginData.icon
  //   : `${kRootPluginsUrl}${pluginData?.icon}`
  ...
  return pluginData ? (
    ...
    >
      <div className="plugin-selection">
        ...
        {/* <img className="plugin-selection-icon" src={iconUrl} /> */}
        <span className="plugin-selection-title">{pluginData.title}</span>
      </div>
    </MenuItem>
  ) : <MenuDivider/>
...
const PluginGroupMenu = observer(function PluginGroupMenu({
  ...
  return (
    <>
      ...
      >
        <span>{title}</span>
        <RightArrow className="plugin-group-menu-arrow" />
      </MenuItem>
    </>
  )
})
