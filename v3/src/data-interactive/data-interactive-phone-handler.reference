...
import { ActionName, DIHandler, DIResources, DIResourceSelector, maybeString, messageCallback } from "./data-interactive-types";
...
function serializeItem(dataSet: IDataSet, item) {
  if (!item) {
    return
  }
  const ret: SerializedItem = {}
  const values: ValuesType = {}
  const baseCollectionID = dataSet.getPath('baseCollection.id')
  dataSet.attributes.forEach(attr => {
    const value = item.getValue(attr.id, baseCollectionID)
 ...
 * @extends SC.Object
 */
class DataInteractivePhoneHandler {
   ...
   */
  sendMessage(message: object, callback?: messageCallback) {
    this.rpcEndpoint.call(message, callback)
  }

  handlerMap: Record<string, DIHandler> | null = null
  ...
  destroy() {
    ...
    const self = this
    const dataInteractiveLogMonitor = appState.document.dataInteractiveLogMonitor
    dataInteractiveLogMonitor.logMonitors = dataInteractiveLogMonitor.logMonitors.filter(logMonitor => {
   ...
   */
  requestDataInteractiveState(callback?: messageCallback) {
    this.sendMessage({
      action: 'get',
   ...
   */
  resolveResources(resourceSelector: DIResourceSelector, action: ActionName) {
    // TODO any type
   ...
   */
  validateValues(iDataContext, iValues) {
    if( SC.none( iValues))
      return;
  ...
  filterResultValues(resultValues) {
    var maxLevels = 10;
   ...
   */
  handleInteractiveFrame: DIHandler = {
   ...
   *
   */
  handleDataContextFromURL: DIHandler = {
    create: function (iResources, iValues) {
      var tURL;
      var status = true;
      if (iValues.URL) {
   ...
   *
   */
  handleDataContextList: DIHandler = {
    get: function (iResources) {
   ...
   *
   */
  handleCollectionList: DIHandler = {
    get: function (iResources) {
      var context = iResources.dataContext;
  ...
  handleAttributeLocation: DIHandler = {
    /*
   ...
   */
  makeSerializableCase(iCollection, iCase) {
    var values = {};
  ...
  handleCaseByIndexOrID: DIHandler = {
    get: function (iResources) {
  ...
  handleCaseFormulaSearch: DIHandler = {
    get: function (iResources) {
      if (!iResources.collection) {
        return {success: false, values: {error: 'Collection not found'}};
      }
  ...
  handleComponent: DIHandler = (function () {
  ...
  handleLogMessageMonitor: DIHandler = {
  ...
  handleUndoChangeNotice: DIHandler = {
    /**
