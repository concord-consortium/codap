...
describe("DataInteractive AllCasesHandler", () => {
  ...
  it("get works as expected", () => {
    ...
    const getCase = (result: DIHandlerFnResult, index: number) =>
      (result.values! as DIAllCases).cases![index].case!
    ...
    const result1 = handler.get?.({ dataContext: dataset, collection: c1 })
    ...
    const result2 = handler.get?.({ dataContext: dataset, collection: c2 })
    ...
    const result3 = handler.get?.({ dataContext: dataset, collection: dataset?.ungrouped })
