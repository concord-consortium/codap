...
import {
  CodapV2Context, ICodapV2DocumentJson, ICodapV2GameContext, isCodapV2Document,
...
export function resolveDocument(iDocContents: any, iMetadata: IDocumentMetadata): Promise<IDocumentModelSnapshot> {
  return new Promise(function (resolve, reject) {
    ...
    if (!iDocContents) {
      ...
      v2DocContentsPromise(iDocContents).then(
        function (contents) {
          // check if this is a valid CODAP document
          const docContents = validateV2Document(contents)
          if (!docContents) {
            reject(t('DG.AppController.openDocument.error.invalid_format'))
          } else {
            const v2Document = new CodapV2Document(docContents, metadata)
            const v3Document = importV2Document(v2Document)
            resolve(getSnapshot(v3Document))
          }
        },
        function (message) {
          reject(message)
